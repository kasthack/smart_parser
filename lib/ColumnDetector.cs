...
namespace Smart.Parser.Lib
{
    ...
    public class ColumnDetector
    {
        ...
        public static bool GetValuesFromTitle(string text, ref string title, ref int? year, ref string ministry)
        {
            ...
            text = Regex.Replace(text, "8\\s+июля\\s+2013", string.Empty);
        ...
        private static int ProcessTitle(IAdapter adapter, ColumnOrdering columnOrdering)
        {
            ...
            while (true)
            {
                ...
                if (row >= adapter.GetRowsCount())
                {
                    row = 0;
                    break;
                    throw new ColumnDetectorException("Headers not found");
                }
        ...
        private static void FixMissingSubheadersForVehicle(IAdapter adapter, ColumnOrdering columnOrdering)
        {
            ...
            var cleanHeader = headerCell.Text.ToLower().Replace(" ", string.Empty);
        ...
        {
            foreach (var (text, country, square, type) in new[] {
                (DeclarationField.MixedColumnWithNaturalText, DeclarationField.MixedRealEstateCountry, DeclarationField.MixedRealEstateSquare, DeclarationField.MixedRealEstateType),
                (DeclarationField.StateColumnWithNaturalText, DeclarationField.StatePropertyCountry, DeclarationField.StatePropertySquare, DeclarationField.StatePropertyType),
                (DeclarationField.OwnedColumnWithNaturalText, DeclarationField.OwnedRealEstateCountry, DeclarationField.OwnedRealEstateSquare, DeclarationField.OwnedRealEstateType),
            })
            {
                FixBadColumnName01_Template(c, text, country, square, type);
            }
        }
        ...
        public static ColumnOrdering ExamineTableBeginning(IAdapter adapter)
        {
        ...
        public static List<Cell> GetColumnCells(IAdapter adapter, int headerStartRow, out int headerEndRow)
        {
        ...
        public static void MapStringsToConstants(IAdapter adapter, List<Cell> cells, ColumnOrdering columnOrdering)
        {
            foreach (var cell in cells)
            {
                var text = cell.GetText(true);
                Logger.Debug($"column title: \"{text.ReplaceEolnWithSpace().CoalesceWhitespace()}\"[{cell.CellWidth}]");
                DeclarationField field;
                var clean_text = AbsenceMarkers.Aggregate(text, (x, y) => x.Replace(y, string.Empty)).Trim();
                ...
                else
                {
                    ...
                    {
                        throw new SmartParserException($"Cannot recognize field \"{text.Replace('\n', ' ')}\"");
                    }
                }

                if (field == DeclarationField.None && !DataHelper.IsEmptyValue(text))
                {
                    throw new ColumnDetectorException($"Fail to detect column type row: {cell.Row} title:{text}");
                }
        ...
        public static void ReadHeader(IAdapter adapter, int headerStartRow, ColumnOrdering columnOrdering)
        {
